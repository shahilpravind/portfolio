...
<?php
    ...
    if (!empty($signout_set))
    {
        ...
        while ($signout = mysqli_fetch_assoc($signout_set))
        {
            ...
            $report .= "<a href=\"form_edit_signout.php?son=" . urlencode($signout["signout_num"] ) ."\">Edit SignOut</a>";
            $report .= "</td>";

            $report .= "<td style=\"text-align: center;\"> ";
            $report .= "<a href=\"delete_signout.php?signout_num=" . urlencode($signout["signout_num"])  . "\">Delete SignOut</a>";
            $report .= "</td>";
